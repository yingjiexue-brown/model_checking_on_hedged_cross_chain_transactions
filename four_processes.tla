...
variables
    ...
    swap_contract = [A2B |->[balance |->100, timeout |-> -1, state |-> INIT,deadline |-> 6],
    ...
    premium_publish_contract = [A2B |->[balance |->10, timeout |-> -1, state |-> INIT,deadline |-> 0],
...
define
 ...
 compensated == /\ \A x\in PARTIES:  ended/\swap_contract[party_contract_map[x]].state=REFUNDED/\conforming[x]=>wallet[x].balance>=wallet[x].input+1
 compensated_BA == ended /\ conforming["BOB"]/\swap_contract["B2A"].state = REFUNDED =>wallet["BOB"].balance>=wallet["BOB"].input+1
 contractliveness == (ended /\ \A x\in PARTIES: conforming[x]) => (\A x \in SWPCN, y \in PATHSIGS: swap_contract[x].state=REDEEMED/\premium_redeem_contract_sa[y].state = REFUNDED/\premium_publish_contract[x].state =REFUNDED2)
...
P_P_AB: \*clock =0, Alice publishes premium_publish(A,B)
    
    if clock<= premium_publish_contract["A2B"].deadline /\ premium_publish_contract["A2B"].state=INIT then 
               premium_publish_contract["A2B"].state:=ESCROW 
              ||premium_publish_contract["A2B"].timeout:=premium_redeem_contract_sa["BCA_ON_AB"].deadline;\* since premium_redeem_contract_sa["BCA_ON_AB"].deadline=5
...
P_R_SA_BA_ON_AB: \* clock =4, Bob publishes premium_redeem(sa,ba) on ab
 if  step_taken[SP_P_AB]/\step_taken[SP_R_SA_A_ON_BA]/\clock<= premium_redeem_contract_sa["BA_ON_AB"].deadline /\ premium_redeem_contract_sa["BA_ON_AB"].state=INIT then 
       premium_redeem_contract_sa["BA_ON_AB"].state:=ESCROW 
              ...
              if premium_publish_contract["A2B"].state=ESCROW then
              premium_publish_contract["A2B"].state:=ACTIVATED
              ||premium_publish_contract["A2B"].timeout := swap_contract["A2B"].deadline;\* deadline for B2A to publish
              end if;
  end if;
...
P_R_SA_BCA_ON_AB: \* clock =5, bob publishes premium_redeem(sa,bca) on AB
 if  step_taken[SP_P_AB]/\step_taken[SP_R_SA_CA_ON_BC]/\clock<= premium_redeem_contract_sa["BCA_ON_AB"].deadline /\ premium_redeem_contract_sa["BCA_ON_AB"].state=INIT then 
       premium_redeem_contract_sa["BCA_ON_AB"].state:=ESCROW 
              ...
              if premium_publish_contract["A2B"].state=ESCROW then
              premium_publish_contract["A2B"].state:=ACTIVATED
              ||premium_publish_contract["A2B"].timeout := swap_contract["A2B"].deadline;\*A2B should publish before 6
              end if;
  elsif premium_publish_contract["A2B"].state=ESCROW then
          premium_publish_contract["A2B"].state:=REFUNDED;
  end if;
...
AB: \*clock =6, Alice publishes (A,B)
    ...
    if premium_publish_contract["A2B"].state=ACTIVATED/\clock<= swap_contract["A2B"].deadline /\  swap_contract["A2B"].state=INIT then 
       swap_contract["A2B"].state:=ESCROW 
              || swap_contract["A2B"].timeout:=path_signature_sa["BCA_ON_AB"].timeout;\*path sig should publish before 11
              step_taken[SAB]:= TRUE;
              premium_publish_contract["A2B"].state:=REFUNDED2;
             
     elsif premium_publish_contract["A2B"].state=ACTIVATED then
           premium_publish_contract["A2B"].state:=LOST; 
           wallet["ALICE"].balance:= wallet["ALICE"].balance -  premium_publish_contract["A2B"].balance
           ||wallet["BOB"].balance := wallet["BOB"].balance +  premium_publish_contract["A2B"].balance        
    end if;
    step_considered[SAB]:= TRUE;
    \* determine conformity
    if premium_publish_contract["A2B"].state>=ACTIVATED /\~step_taken[SAB] then 
       conforming["ALICE"]:=FALSE;
    end if;
...
SA_BA_ON_AB: \* clock =10, BOB publishes (sa,Ba) on AB
 if  step_taken[SAB]/\step_taken[SSA_A_ON_BA]/\clock<= path_signature_sa["BA_ON_AB"].timeout /\ path_signature_sa["BA_ON_AB"].state=INIT then 
       path_signature_sa["BA_ON_AB"].state:=RELEASED;
             ...
             if swap_contract["A2B"].state = ESCROW then 
             swap_contract["A2B"].state := REDEEMED;
             wallet["BOB"].balance:= wallet["BOB"].balance + swap_contract["A2B"].balance
             ||wallet["ALICE"].balance := wallet["ALICE"].balance - swap_contract["A2B"].balance;
             end if;
...
SA_BCA_ON_AB: \* clock =9, bob publishes premium_redeem(sa,bca) on AB
 if step_taken[SAB]/\step_taken[SSA_CA_ON_BC]/\clock<= path_signature_sa["BCA_ON_AB"].timeout /\ path_signature_sa["BCA_ON_AB"].state=INIT then 
       path_signature_sa["BCA_ON_AB"].state:=RELEASED; 
        ...
        if swap_contract["A2B"].state = ESCROW  then
           swap_contract["A2B"].state := REFUNDED;
        end if;
        if premium_redeem_contract_sa["BCA_ON_AB"].state = ESCROW then
...
P_P_BC: \*clock =1, Bob publishes premium_publish(B,C)
  if clock<= premium_publish_contract["B2C"].deadline /\ premium_publish_contract["B2C"].state=INIT then 
       premium_publish_contract["B2C"].state:=ESCROW 
              ||premium_publish_contract["B2C"].timeout:=premium_redeem_contract_sa["CA_ON_BC"].deadline;\* premium_redeem_contract_sa["CA_ON_BC"].deadline =4
   ...
   \*determine conformity
   if ~step_considered[SP_P_AB] /\ clock<=premium_publish_contract["A2B"].deadline then
       conforming["BOB"]:=FALSE;
...
P_R_SA_CA_ON_BC: \* clock =4, carol publishes premium_redeem(sa,ca) on bc
 if  step_taken[SP_P_BC]/\step_taken[SP_R_SA_A_ON_CA]/\clock<= premium_redeem_contract_sa["CA_ON_BC"].deadline /\ premium_redeem_contract_sa["CA_ON_BC"].state=INIT then 
       premium_redeem_contract_sa["CA_ON_BC"].state:=ESCROW 
              ...
              if premium_publish_contract["B2C"].state=ESCROW then
              premium_publish_contract["B2C"].state:=ACTIVATED
              ||premium_publish_contract["B2C"].timeout := swap_contract["B2C"].deadline;\* deadline for B2C to publish
              end if;
  elsif premium_publish_contract["B2C"].state=ESCROW then
        premium_publish_contract["B2C"].state:=REFUNDED;
  end if;
 ...
 BC: \*clock =7, Bob publishes (B,C)
  if premium_publish_contract["B2C"].state=ACTIVATED/\clock<= swap_contract["B2C"].deadline /\ swap_contract["B2C"].state=INIT then 
       swap_contract["B2C"].state:=ESCROW 
              ||swap_contract["B2C"].timeout:=path_signature_sa["CA_ON_BC"].timeout;\* should be redeemed before 10
              step_taken[SBC]:= TRUE;
          premium_publish_contract["B2C"].state:=REFUNDED2;
  elsif premium_publish_contract["B2C"].state=ACTIVATED then
           premium_publish_contract["B2C"].state:=LOST; 
           wallet["CAROL"].balance := wallet["CAROL"].balance + premium_publish_contract["B2C"].balance
          ||wallet["BOB"].balance := wallet["BOB"].balance - premium_publish_contract["B2C"].balance
           
   
   end if;
  step_considered[SBC]:= TRUE;
  \*determine conformity
   if  ~step_considered[SAB]/\clock<=swap_contract["A2B"].deadline then
        conforming["BOB"]:=FALSE;
   elsif step_taken[SAB]/\premium_publish_contract["B2C"].state>=ACTIVATED /\~step_taken[SBC] then 
       conforming["BOB"]:=FALSE;
...
SA_CA_ON_BC: \* clock =10, carol publishes (sa,ca) on bc
 if step_taken[SBC]/\ step_taken[SSA_A_ON_CA]/\clock<= path_signature_sa["CA_ON_BC"].timeout /\ path_signature_sa["CA_ON_BC"].state=INIT then 
       path_signature_sa["CA_ON_BC"].state:=RELEASED;
             ...
             if swap_contract["B2C"].state = ESCROW then 
             swap_contract["B2C"].state := REDEEMED;
             wallet["CAROL"].balance:= wallet["CAROL"].balance + swap_contract["B2C"].balance
             ||wallet["BOB"].balance := wallet["BOB"].balance - swap_contract["B2C"].balance;
             end if;
        ...
        if swap_contract["B2C"].state = ESCROW  then
           swap_contract["B2C"].state := REFUNDED;
        end if;
...
P_P_CA: \*clock =0, Carol publishes premium_publish(C,A) 
    if clock<= premium_publish_contract["C2A"].deadline /\ premium_publish_contract["C2A"].state=INIT then 
       premium_publish_contract["C2A"].state:=ESCROW 
              ||premium_publish_contract["C2A"].timeout:=premium_redeem_contract_sa["A_ON_CA"].deadline;\*since premium_redeem_contract_sa["A_ON_CA"].deadline=3
   ...
   \* determine conformity 
   if ~step_considered[SP_P_BC]/\ clock<=premium_publish_contract["B2C"].deadline then 
      conforming["CAROL"]:=FALSE;
...
P_R_SA_A_ON_CA: \* clock =3, Alice publishes premium_redeem(sa,a) on CA
 if  step_taken[SP_P_CA]/\clock<= premium_redeem_contract_sa["A_ON_CA"].deadline /\ premium_redeem_contract_sa["A_ON_CA"].state=INIT then 
       premium_redeem_contract_sa["A_ON_CA"].state:=ESCROW 
              ...
              if premium_publish_contract["C2A"].state=ESCROW then
              premium_publish_contract["C2A"].state:=ACTIVATED
              ||premium_publish_contract["C2A"].timeout := swap_contract["C2A"].deadline;\* deadline for C2A to publish
              end if;
  elsif premium_publish_contract["C2A"].state=ESCROW then
        premium_publish_contract["C2A"].state:=REFUNDED;
  else
       skip;
  end if;
  step_considered[SP_R_SA_A_ON_CA]:= TRUE;
  if  ~(step_considered[SP_P_CA]/\step_considered[SP_P_BA])\/clock<=premium_publish_contract["C2A"].deadline then
      conforming["ALICE"]:=FALSE;
...
CA: \*clock =8, Carol publishes (C,A)
    if premium_publish_contract["C2A"].state=ACTIVATED/\clock<= swap_contract["C2A"].deadline /\ swap_contract["C2A"].state=INIT then 
       swap_contract["C2A"].state:=ESCROW 
              ||swap_contract["C2A"].timeout:=path_signature_sa["A_ON_CA"].timeout;\*should be redeemed before 9
              step_taken[SCA]:= TRUE;
              premium_publish_contract["C2A"].state:=REFUNDED2;
           
     elsif premium_publish_contract["C2A"].state=ACTIVATED then
           premium_publish_contract["C2A"].state:=LOST;
           wallet["ALICE"].balance := wallet["ALICE"].balance + premium_publish_contract["C2A"].balance
           ||wallet["CAROL"].balance := wallet["CAROL"].balance - premium_publish_contract["C2A"].balance  
    end if;
   step_considered[SCA]:= TRUE;
   \* determine conformity
    if ~step_considered[SBC]/\clock<=swap_contract["B2C"].deadline then
     conforming["CAROL"]:=FALSE;
    elsif ~step_taken[SBC]/\step_taken[SCA] then 
        conforming["CAROL"]:=FALSE;
    elsif step_taken[SBC]/\premium_publish_contract["C2A"].state>=ACTIVATED /\~step_taken[SCA] then 
       conforming["CAROL"]:=FALSE;
    end if;
...
SA_A_ON_CA: \* clock =9, Alice publishes (sa,a) on CA
 if step_taken[SCA]/\clock<= path_signature_sa["A_ON_CA"].timeout /\ path_signature_sa["A_ON_CA"].state=INIT then 
       path_signature_sa["A_ON_CA"].state:=RELEASED;
             ...
             if swap_contract["C2A"].state = ESCROW then  
             swap_contract["C2A"].state := REDEEMED;
             wallet["CAROL"].balance:= wallet["CAROL"].balance - swap_contract["C2A"].balance
             ||wallet["ALICE"].balance := wallet["ALICE"].balance + swap_contract["C2A"].balance;
             end if;
        ...
        if swap_contract["C2A"].state = ESCROW  then
           swap_contract["C2A"].state := REFUNDED;
        end if;
 ...
 if ~(step_considered[SCA]\/step_considered[SBA])/\clock<=swap_contract["C2A"].deadline then
     conforming["ALICE"]:=FALSE;
...
P_P_BA: \*clock =1, Bob publishes premium_publish(B,A)
    if  clock<= premium_publish_contract["B2A"].deadline /\ premium_publish_contract["B2A"].state=INIT then 
               premium_publish_contract["B2A"].state:=ESCROW 
              ||premium_publish_contract["B2A"].timeout:= premium_redeem_contract_sa["A_ON_BA"].deadline;\* since premium_redeem_contract_sa["A_ON_AB"].deadline=3
...
P_R_SA_A_ON_BA: \* clock =3, Alice publishes premium_redeem(sa,a) on BA
 if  step_taken[SP_P_BA]/\clock<= premium_redeem_contract_sa["A_ON_BA"].deadline /\ premium_redeem_contract_sa["A_ON_BA"].state=INIT then 
       premium_redeem_contract_sa["A_ON_BA"].state:=ESCROW 
              ...
              if premium_publish_contract["B2A"].state=ESCROW then
              premium_publish_contract["B2A"].state:=ACTIVATED
              ||premium_publish_contract["B2A"].timeout := swap_contract["B2A"].deadline;\* deadline for C2A to publish
              end if;
  elsif premium_publish_contract["B2A"].state=ESCROW then
        premium_publish_contract["B2A"].state:=REFUNDED;
  end if;
 ...
 BA: \*clock =7, Bob publishes (B,A)
  ...
  if premium_publish_contract["B2A"].state=ACTIVATED/\clock<= swap_contract["B2A"].deadline /\ swap_contract["B2A"].state=INIT then 
       swap_contract["B2A"].state:=ESCROW 
              ||swap_contract["B2A"].timeout:=path_signature_sa["A_ON_BA"].timeout;\* should be redeemed before 10
              step_taken[SBA]:= TRUE;
          premium_publish_contract["B2A"].state:=REFUNDED2;

     elsif premium_publish_contract["B2A"].state=ACTIVATED then
           premium_publish_contract["B2A"].state:=LOST; 
           wallet["ALICE"].balance := wallet["ALICE"].balance + premium_publish_contract["B2C"].balance
          ||wallet["BOB"].balance := wallet["BOB"].balance - premium_publish_contract["B2A"].balance;
    end if;
  ...
  \*determine conformity
   ...
   if ~step_considered[SAB]/\clock<=swap_contract["A2B"].deadline then
       conforming["BOB"]:=FALSE;
   elsif step_taken[SAB]/\premium_publish_contract["B2A"].state>=ACTIVATED /\~step_taken[SBA] then 
       conforming["BOB"]:=FALSE;
 ...
 if step_taken[SBA]/\clock<= path_signature_sa["A_ON_BA"].timeout /\ path_signature_sa["A_ON_BA"].state=INIT then 
       path_signature_sa["A_ON_BA"].state:=RELEASED;
             ...
             if swap_contract["B2A"].state = ESCROW then  
             swap_contract["B2A"].state := REDEEMED;
             wallet["BOB"].balance:= wallet["BOB"].balance - swap_contract["B2A"].balance
             ||wallet["ALICE"].balance := wallet["ALICE"].balance + swap_contract["B2A"].balance;
             end if;
        ...
        if swap_contract["B2A"].state = ESCROW  then
           swap_contract["B2A"].state := REFUNDED;
        end if;
...
\* BEGIN TRANSLATION - the hash of the PCal code: PCal-a7b97ae13023e62c659e911d65074359
VARIABLES swap_contract, premium_publish_contract, premium_redeem_contract_sa, 
...
vars == << swap_contract, premium_publish_contract, 
...
P_P_AB == /\ pc[A2B] = "P_P_AB"
          /\ IF clock<= premium_publish_contract["A2B"].deadline /\ premium_publish_contract["A2B"].state=INIT
                THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["A2B"].state = ESCROW,
                ...
                ELSE /\ conforming' = [conforming EXCEPT !["ALICE"] = FALSE]
                     /\ UNCHANGED << premium_publish_contract, step_taken >>
          /\ step_considered' = [step_considered EXCEPT ![SP_P_AB] = TRUE]
          /\ pc' = [pc EXCEPT ![A2B] = "P_R_SA_BA_ON_AB"]
          /\ UNCHANGED << swap_contract, premium_redeem_contract_sa, 
...
P_R_SA_BA_ON_AB == /\ pc[A2B] = "P_R_SA_BA_ON_AB"
                   /\ IF step_taken[SP_P_AB]/\step_taken[SP_R_SA_A_ON_BA]/\clock<= premium_redeem_contract_sa["BA_ON_AB"].deadline /\ premium_redeem_contract_sa["BA_ON_AB"].state=INIT
                         THEN /\ premium_redeem_contract_sa' = [premium_redeem_contract_sa EXCEPT !["BA_ON_AB"].state = ESCROW,
                              ...
                              /\ IF premium_publish_contract["A2B"].state=ESCROW
                                    THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["A2B"].state = ACTIVATED,
                                                                                                         !["A2B"].timeout = swap_contract["A2B"].deadline]
                                    ELSE /\ TRUE
                                         /\ UNCHANGED premium_publish_contract
                         ELSE /\ TRUE
                              /\ UNCHANGED << premium_publish_contract, 
                   ...
                   /\ UNCHANGED << swap_contract, path_signature_sa, wallet, 
                    ...
                    /\ IF step_taken[SP_P_AB]/\step_taken[SP_R_SA_CA_ON_BC]/\clock<= premium_redeem_contract_sa["BCA_ON_AB"].deadline /\ premium_redeem_contract_sa["BCA_ON_AB"].state=INIT
                          THEN /\ premium_redeem_contract_sa' = [premium_redeem_contract_sa EXCEPT !["BCA_ON_AB"].state = ESCROW,
                               ...
                               /\ IF premium_publish_contract["A2B"].state=ESCROW
                                     ...
                                     ELSE /\ TRUE
                                          /\ UNCHANGED premium_publish_contract
                          ELSE /\ IF premium_publish_contract["A2B"].state=ESCROW
                                     THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["A2B"].state = REFUNDED]
                                     ELSE /\ TRUE
                                          /\ UNCHANGED premium_publish_contract
...
AB == /\ pc[A2B] = "AB"
      /\ IF premium_publish_contract["A2B"].state=ACTIVATED/\clock<= swap_contract["A2B"].deadline /\  swap_contract["A2B"].state=INIT
            THEN /\ swap_contract' = [swap_contract EXCEPT !["A2B"].state = ESCROW,
                                                           !["A2B"].timeout = path_signature_sa["BCA_ON_AB"].timeout]
                 /\ step_taken' = [step_taken EXCEPT ![SAB] = TRUE]
                 /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["A2B"].state = REFUNDED2]
                 /\ UNCHANGED wallet
            ELSE /\ IF premium_publish_contract["A2B"].state=ACTIVATED
                       THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["A2B"].state = LOST]
                            /\ wallet' = [wallet EXCEPT !["ALICE"].balance = wallet["ALICE"].balance -  premium_publish_contract'["A2B"].balance,
                                                        !["BOB"].balance = wallet["BOB"].balance +  premium_publish_contract'["A2B"].balance]
                       ELSE /\ TRUE
                            /\ UNCHANGED << premium_publish_contract, wallet >>
                 /\ UNCHANGED << swap_contract, step_taken >>
      /\ step_considered' = [step_considered EXCEPT ![SAB] = TRUE]
      /\ IF premium_publish_contract'["A2B"].state>=ACTIVATED /\~step_taken'[SAB]
...
SA_BA_ON_AB == /\ pc[A2B] = "SA_BA_ON_AB"
               /\ IF step_taken[SAB]/\step_taken[SSA_A_ON_BA]/\clock<= path_signature_sa["BA_ON_AB"].timeout /\ path_signature_sa["BA_ON_AB"].state=INIT
                     THEN /\ path_signature_sa' = [path_signature_sa EXCEPT !["BA_ON_AB"].state = RELEASED]
                          ...
                          /\ IF swap_contract["A2B"].state = ESCROW
                                THEN /\ swap_contract' = [swap_contract EXCEPT !["A2B"].state = REDEEMED]
                                     /\ wallet' = [wallet EXCEPT !["BOB"].balance = wallet["BOB"].balance + swap_contract'["A2B"].balance,
                                                                 !["ALICE"].balance = wallet["ALICE"].balance - swap_contract'["A2B"].balance]
                                ELSE /\ TRUE
                                     /\ UNCHANGED << swap_contract, wallet >>
                     ...
                     ELSE /\ IF step_taken[SAB]/\premium_redeem_contract_sa["BA_ON_AB"].state = ESCROW
                                ...
                                ELSE /\ TRUE
                                     /\ UNCHANGED << premium_redeem_contract_sa, 
                                                     wallet >>
                          /\ UNCHANGED << swap_contract, path_signature_sa, 
                                          step_taken >>
               ...
               /\ UNCHANGED << premium_publish_contract, clock, ending, 
                               party_contract_map >>
...
SA_BCA_ON_AB == /\ pc[A2B] = "SA_BCA_ON_AB"
                /\ IF step_taken[SAB]/\step_taken[SSA_CA_ON_BC]/\clock<= path_signature_sa["BCA_ON_AB"].timeout /\ path_signature_sa["BCA_ON_AB"].state=INIT
                      THEN /\ path_signature_sa' = [path_signature_sa EXCEPT !["BCA_ON_AB"].state = RELEASED]
                           ...
                           /\ IF swap_contract["A2B"].state = ESCROW
                                 THEN /\ swap_contract' = [swap_contract EXCEPT !["A2B"].state = REDEEMED]
                                      /\ wallet' = [wallet EXCEPT !["ALICE"].balance = wallet["ALICE"].balance - swap_contract'["A2B"].balance,
                                                                  !["BOB"].balance = wallet["BOB"].balance + swap_contract'["A2B"].balance]
                                 ELSE /\ TRUE
                                      /\ UNCHANGED << swap_contract, wallet >>
                           /\ IF premium_redeem_contract_sa["BCA_ON_AB"].state = ESCROW
                                 ...
                                 ELSE /\ TRUE
                                      /\ UNCHANGED premium_redeem_contract_sa
                      ELSE /\ IF swap_contract["A2B"].state = ESCROW
                                 THEN /\ swap_contract' = [swap_contract EXCEPT !["A2B"].state = REFUNDED]
                                 ELSE /\ TRUE
                                      /\ UNCHANGED swap_contract
                           /\ IF premium_redeem_contract_sa["BCA_ON_AB"].state = ESCROW
                ...
                /\ UNCHANGED << premium_publish_contract, clock, 
                                party_contract_map >>
...
P_P_BC == /\ pc[B2C] = "P_P_BC"
          /\ IF clock<= premium_publish_contract["B2C"].deadline /\ premium_publish_contract["B2C"].state=INIT
                THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2C"].state = ESCROW,
          ...
          /\ IF ~step_considered'[SP_P_AB] /\ clock<=premium_publish_contract'["A2B"].deadline
                THEN /\ conforming' = [conforming EXCEPT !["BOB"] = FALSE]
...
P_R_SA_CA_ON_BC == /\ pc[B2C] = "P_R_SA_CA_ON_BC"
                   /\ IF step_taken[SP_P_BC]/\step_taken[SP_R_SA_A_ON_CA]/\clock<= premium_redeem_contract_sa["CA_ON_BC"].deadline /\ premium_redeem_contract_sa["CA_ON_BC"].state=INIT
                         THEN /\ premium_redeem_contract_sa' = [premium_redeem_contract_sa EXCEPT !["CA_ON_BC"].state = ESCROW,
                              ...
                              /\ IF premium_publish_contract["B2C"].state=ESCROW
                                    THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2C"].state = ACTIVATED,
                                                                                                         !["B2C"].timeout = swap_contract["B2C"].deadline]
                                    ELSE /\ TRUE
                                         /\ UNCHANGED premium_publish_contract
                         ELSE /\ IF premium_publish_contract["B2C"].state=ESCROW
                                    THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2C"].state = REFUNDED]
                                    ELSE /\ TRUE
                                         /\ UNCHANGED premium_publish_contract
                              /\ UNCHANGED << premium_redeem_contract_sa, 
                                              step_taken >>
...
BC == /\ pc[B2C] = "BC"
      /\ IF premium_publish_contract["B2C"].state=ACTIVATED/\clock<= swap_contract["B2C"].deadline /\ swap_contract["B2C"].state=INIT
            THEN /\ swap_contract' = [swap_contract EXCEPT !["B2C"].state = ESCROW,
                                                           !["B2C"].timeout = path_signature_sa["CA_ON_BC"].timeout]
                 /\ step_taken' = [step_taken EXCEPT ![SBC] = TRUE]
                 /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2C"].state = REFUNDED2]
                 /\ UNCHANGED wallet
            ELSE /\ IF premium_publish_contract["B2C"].state=ACTIVATED
                       THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2C"].state = LOST]
                            /\ wallet' = [wallet EXCEPT !["CAROL"].balance = wallet["CAROL"].balance + premium_publish_contract'["B2C"].balance,
                                                        !["BOB"].balance = wallet["BOB"].balance - premium_publish_contract'["B2C"].balance]
                       ELSE /\ TRUE
                            /\ UNCHANGED << premium_publish_contract, wallet >>
                 /\ UNCHANGED << swap_contract, step_taken >>
      /\ step_considered' = [step_considered EXCEPT ![SBC] = TRUE]
      /\ IF ~step_considered'[SAB]/\clock<=swap_contract'["A2B"].deadline
            THEN /\ conforming' = [conforming EXCEPT !["BOB"] = FALSE]
            ELSE /\ IF step_taken'[SAB]/\premium_publish_contract'["B2C"].state>=ACTIVATED /\~step_taken'[SBC]
                       THEN /\ conforming' = [conforming EXCEPT !["BOB"] = FALSE]
...
SA_CA_ON_BC == /\ pc[B2C] = "SA_CA_ON_BC"
               /\ IF step_taken[SBC]/\ step_taken[SSA_A_ON_CA]/\clock<= path_signature_sa["CA_ON_BC"].timeout /\ path_signature_sa["CA_ON_BC"].state=INIT
                     THEN /\ path_signature_sa' = [path_signature_sa EXCEPT !["CA_ON_BC"].state = RELEASED]
                          ...
                          /\ IF swap_contract["B2C"].state = ESCROW
                                THEN /\ swap_contract' = [swap_contract EXCEPT !["B2C"].state = REDEEMED]
                                     /\ wallet' = [wallet EXCEPT !["CAROL"].balance = wallet["CAROL"].balance + swap_contract'["B2C"].balance,
                                                                 !["BOB"].balance = wallet["BOB"].balance - swap_contract'["B2C"].balance]
                                ELSE /\ TRUE
                                     /\ UNCHANGED << swap_contract, wallet >>
                          /\ IF premium_redeem_contract_sa["CA_ON_BC"].state = ESCROW
                                ...
                                ELSE /\ TRUE
                                     /\ UNCHANGED premium_redeem_contract_sa
                     ELSE /\ IF swap_contract["B2C"].state = ESCROW
                                THEN /\ swap_contract' = [swap_contract EXCEPT !["B2C"].state = REFUNDED]
                                ELSE /\ TRUE
                                     /\ UNCHANGED swap_contract
...
P_P_CA == /\ pc[C2A] = "P_P_CA"
          /\ IF clock<= premium_publish_contract["C2A"].deadline /\ premium_publish_contract["C2A"].state=INIT
                THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["C2A"].state = ESCROW,
          ...
          /\ IF ~step_considered'[SP_P_BC]/\ clock<=premium_publish_contract'["B2C"].deadline
                THEN /\ conforming' = [conforming EXCEPT !["CAROL"] = FALSE]
...
P_R_SA_A_ON_CA == /\ pc[C2A] = "P_R_SA_A_ON_CA"
                  /\ IF step_taken[SP_P_CA]/\clock<= premium_redeem_contract_sa["A_ON_CA"].deadline /\ premium_redeem_contract_sa["A_ON_CA"].state=INIT
                        THEN /\ premium_redeem_contract_sa' = [premium_redeem_contract_sa EXCEPT !["A_ON_CA"].state = ESCROW,
                             ...
                             /\ IF premium_publish_contract["C2A"].state=ESCROW
                                   THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["C2A"].state = ACTIVATED,
                                                                                                        !["C2A"].timeout = swap_contract["C2A"].deadline]
                                   ELSE /\ TRUE
                                        /\ UNCHANGED premium_publish_contract
                        ELSE /\ IF premium_publish_contract["C2A"].state=ESCROW
                                   THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["C2A"].state = REFUNDED]
                                   ELSE /\ TRUE
                                        /\ UNCHANGED premium_publish_contract
                             /\ UNCHANGED << premium_redeem_contract_sa, 
                                             step_taken >>
                  /\ step_considered' = [step_considered EXCEPT ![SP_R_SA_A_ON_CA] = TRUE]
                  /\ IF ~(step_considered'[SP_P_CA]/\step_considered'[SP_P_BA])\/clock<=premium_publish_contract'["C2A"].deadline
                        THEN /\ conforming' = [conforming EXCEPT !["ALICE"] = FALSE]
...
CA == /\ pc[C2A] = "CA"
      /\ IF premium_publish_contract["C2A"].state=ACTIVATED/\clock<= swap_contract["C2A"].deadline /\ swap_contract["C2A"].state=INIT
            THEN /\ swap_contract' = [swap_contract EXCEPT !["C2A"].state = ESCROW,
                                                           !["C2A"].timeout = path_signature_sa["A_ON_CA"].timeout]
                 /\ step_taken' = [step_taken EXCEPT ![SCA] = TRUE]
                 /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["C2A"].state = REFUNDED2]
                 /\ UNCHANGED wallet
            ELSE /\ IF premium_publish_contract["C2A"].state=ACTIVATED
                       THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["C2A"].state = LOST]
                            /\ wallet' = [wallet EXCEPT !["ALICE"].balance = wallet["ALICE"].balance + premium_publish_contract'["C2A"].balance,
                                                        !["CAROL"].balance = wallet["CAROL"].balance - premium_publish_contract'["C2A"].balance]
                       ELSE /\ TRUE
                            /\ UNCHANGED << premium_publish_contract, wallet >>
                 /\ UNCHANGED << swap_contract, step_taken >>
      /\ step_considered' = [step_considered EXCEPT ![SCA] = TRUE]
      /\ IF ~step_considered'[SBC]/\clock<=swap_contract'["B2C"].deadline
            THEN /\ conforming' = [conforming EXCEPT !["CAROL"] = FALSE]
            ELSE /\ IF ~step_taken'[SBC]/\step_taken'[SCA]
                       THEN /\ conforming' = [conforming EXCEPT !["CAROL"] = FALSE]
                       ELSE /\ IF step_taken'[SBC]/\premium_publish_contract'["C2A"].state>=ACTIVATED /\~step_taken'[SCA]
                                  THEN /\ conforming' = [conforming EXCEPT !["CAROL"] = FALSE]
                                  ELSE /\ TRUE
                                       /\ UNCHANGED conforming
...
SA_A_ON_CA == /\ pc[C2A] = "SA_A_ON_CA"
              /\ IF step_taken[SCA]/\clock<= path_signature_sa["A_ON_CA"].timeout /\ path_signature_sa["A_ON_CA"].state=INIT
                    THEN /\ path_signature_sa' = [path_signature_sa EXCEPT !["A_ON_CA"].state = RELEASED]
                         ...
                         /\ IF swap_contract["C2A"].state = ESCROW
                               THEN /\ swap_contract' = [swap_contract EXCEPT !["C2A"].state = REDEEMED]
                                    /\ wallet' = [wallet EXCEPT !["CAROL"].balance = wallet["CAROL"].balance - swap_contract'["C2A"].balance,
                                                                !["ALICE"].balance = wallet["ALICE"].balance + swap_contract'["C2A"].balance]
                               ELSE /\ TRUE
                                    /\ UNCHANGED << swap_contract, wallet >>
                         /\ IF premium_redeem_contract_sa["A_ON_CA"].state = ESCROW
                               ...
                               ELSE /\ TRUE
                                    /\ UNCHANGED premium_redeem_contract_sa
                    ELSE /\ IF swap_contract["C2A"].state = ESCROW
                               THEN /\ swap_contract' = [swap_contract EXCEPT !["C2A"].state = REFUNDED]
                               ELSE /\ TRUE
                                    /\ UNCHANGED swap_contract
              ...
              /\ IF ~(step_considered'[SCA]\/step_considered'[SBA])/\clock<=swap_contract'["C2A"].deadline
                    THEN /\ conforming' = [conforming EXCEPT !["ALICE"] = FALSE]
...
P_P_BA == /\ pc[B2A] = "P_P_BA"
          /\ IF clock<= premium_publish_contract["B2A"].deadline /\ premium_publish_contract["B2A"].state=INIT
                THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2A"].state = ESCROW,
...
P_R_SA_A_ON_BA == /\ pc[B2A] = "P_R_SA_A_ON_BA"
                  /\ IF step_taken[SP_P_BA]/\clock<= premium_redeem_contract_sa["A_ON_BA"].deadline /\ premium_redeem_contract_sa["A_ON_BA"].state=INIT
                        THEN /\ premium_redeem_contract_sa' = [premium_redeem_contract_sa EXCEPT !["A_ON_BA"].state = ESCROW,
                             ...
                             /\ IF premium_publish_contract["B2A"].state=ESCROW
                                   THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2A"].state = ACTIVATED,
                                                                                                        !["B2A"].timeout = swap_contract["B2A"].deadline]
                                   ELSE /\ TRUE
                                        /\ UNCHANGED premium_publish_contract
                        ELSE /\ IF premium_publish_contract["B2A"].state=ESCROW
                                   THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2A"].state = REFUNDED]
                                   ELSE /\ TRUE
                                        /\ UNCHANGED premium_publish_contract
                             /\ UNCHANGED << premium_redeem_contract_sa, 
                                             step_taken >>
...
BA == /\ pc[B2A] = "BA"
      /\ IF premium_publish_contract["B2A"].state=ACTIVATED/\clock<= swap_contract["B2A"].deadline /\ swap_contract["B2A"].state=INIT
            THEN /\ swap_contract' = [swap_contract EXCEPT !["B2A"].state = ESCROW,
                                                           !["B2A"].timeout = path_signature_sa["A_ON_BA"].timeout]
                 /\ step_taken' = [step_taken EXCEPT ![SBA] = TRUE]
                 /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2A"].state = REFUNDED2]
                 /\ UNCHANGED wallet
            ELSE /\ IF premium_publish_contract["B2A"].state=ACTIVATED
                       THEN /\ premium_publish_contract' = [premium_publish_contract EXCEPT !["B2A"].state = LOST]
                            /\ wallet' = [wallet EXCEPT !["ALICE"].balance = wallet["ALICE"].balance + premium_publish_contract'["B2C"].balance,
                                                        !["BOB"].balance = wallet["BOB"].balance - premium_publish_contract'["B2A"].balance]
                       ELSE /\ TRUE
                            /\ UNCHANGED << premium_publish_contract, wallet >>
                 /\ UNCHANGED << swap_contract, step_taken >>
      ...
      /\ IF ~step_considered'[SAB]/\clock<=swap_contract'["A2B"].deadline
            THEN /\ conforming' = [conforming EXCEPT !["BOB"] = FALSE]
            ELSE /\ IF step_taken'[SAB]/\premium_publish_contract'["B2A"].state>=ACTIVATED /\~step_taken'[SBA]
                       THEN /\ conforming' = [conforming EXCEPT !["BOB"] = FALSE]
...
SA_A_ON_BA == /\ pc[B2A] = "SA_A_ON_BA"
              /\ IF step_taken[SBA]/\clock<= path_signature_sa["A_ON_BA"].timeout /\ path_signature_sa["A_ON_BA"].state=INIT
                    THEN /\ path_signature_sa' = [path_signature_sa EXCEPT !["A_ON_BA"].state = RELEASED]
                         ...
                         /\ IF swap_contract["B2A"].state = ESCROW
                               THEN /\ swap_contract' = [swap_contract EXCEPT !["B2A"].state = REDEEMED]
                                    /\ wallet' = [wallet EXCEPT !["BOB"].balance = wallet["BOB"].balance - swap_contract'["B2A"].balance,
                                                                !["ALICE"].balance = wallet["ALICE"].balance + swap_contract'["B2A"].balance]
                               ELSE /\ TRUE
                                    /\ UNCHANGED << swap_contract, wallet >>
                         /\ IF premium_redeem_contract_sa["A_ON_BA"].state = ESCROW
                               ...
                               ELSE /\ TRUE
                                    /\ UNCHANGED premium_redeem_contract_sa
                    ELSE /\ IF swap_contract["B2A"].state = ESCROW
                               THEN /\ swap_contract' = [swap_contract EXCEPT !["B2A"].state = REFUNDED]
                               ELSE /\ TRUE
                                    /\ UNCHANGED swap_contract
...
tick == /\ pc[CLOCK] = "tick"
        ...
        /\ UNCHANGED << swap_contract, premium_publish_contract, 
...
\* END TRANSLATION - the hash of the generated TLA code (remove to silence divergence warnings): TLA-d2abd23fa91c9ab9879ebd2183c9dd83
====
